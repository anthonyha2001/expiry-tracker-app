...
export const InventoryProvider = ({ children }) => {
  ...
  const uploadStockBalanceFile = async (file) => {
    ...
    setItems(result.updatedItems);
    // Let the UI know that an undo is possible
    return { message: result.message, undo: true };
  };

  const undoLastStockUpdate = async () => {
    const response = await fetch(`${API_BASE_URL}/stock-undo`, { method: 'POST' });
    const result = await response.json();
    if (!response.ok) {
        throw new Error(result.message || 'Failed to undo stock update.');
    }
    setItems(result.updatedItems);
